...
int main(int argc, char *argv[]){
  ...
  const long long int npoint = static_cast<long long int>(N_RE)*static_cast<long long int>(N_IM) ;

  if( npoint%VEC_NUM_E != 0 ){
    std::printf("Error:: Num points (%lld) must be divisible by %d.\n", npoint, VEC_NUM_E);
    return 1;
  }
  ...
  std::printf("1 CPU core :: Image size %d x %d :: Elapsed time = %f seconds.\n", N_RE, N_IM, static_cast<float>(std::chrono::duration_cast<std::chrono::microseconds>(end-begin).count())/1000000.0f);
    ...
    if( SAVE_IMAGE ){
        ...
        for(int j=0 ; j<N_IM ; ++j)
        {
          ...
          {
            std::fprintf(outfile,"%d ", nIter[ (long long int)j*N_RE+i ]);
          }
          std::fprintf(outfile,"\n");
        }
        std::fclose(outfile);
