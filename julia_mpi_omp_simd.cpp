...
int main(int argc, char *argv[]){
  ...
  int mpi_id, mpi_num, num_threads ;
  ...
  int N_RE, N_IM ;
  if( argc==3 ){
    N_RE = static_cast<int>(atoi(argv[1])) ;
    N_IM = static_cast<int>(atoi(argv[2])) ;
  }else{
  ...
  long long int npoint = static_cast<long long int>( std::ceil((double)total_point/mpi_num) ) ;
  long long int base   = npoint*mpi_id ;
  ...
  int *nIter = static_cast<int*>(aligned_alloc(VEC_SIZE, sizeof(int)*npoint));
  ...
  #pragma omp for
  for(long long int i=0 ; i<npoint; i+=VEC_NUM_E)
  {
  ...
  if( mpi_id==root ){
    std::printf("%d MPI procs x %d OMP threads :: Image size %d x %d :: Elapsed time = %f seconds.\n", mpi_num, num_threads, N_RE, N_IM, MPI_Wtime()-start_time);
  }
  ...
  if( SAVE_IMAGE ){
    ...
    if( mpi_id==root ){
      //std::printf("Start writing 'julia.pgm'.\n");
      ...
      for(int proc=0 ; proc<mpi_num-1 ; ++proc)
      {
        ...
        //std::printf("Receive Result from MPI process %d.\n", proc);

        //std::printf("-- Start writing result received from MPI process %d.\n", proc);
        ...
        //std::printf("-- Done writing result received from MPI process %d.\n", proc);
      }
      std::free(temp);
      //std::printf("-- Start writing result of MPI process %d.\n", root);
      ...
      //std::printf("Finish writing 'julia.pgm'.\n");
    }else{
